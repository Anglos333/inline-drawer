...
const ALL_MESSAGES_VIRTUAL_MEDIA_SELECTOR = 'img, video, iframe, details, .mes_reasoning_details';
const HISTORY_BACKFILL_SETTLE_MS = 380;
const HISTORY_BACKFILL_TOP_THRESHOLD = 260;
const PROGRAMMATIC_SCROLL_QUIET_MS = 650;

let messageEventHandlers = [];
const chatChangedBound = { value: false };
...
const getSettings = () => extension_settings[EXT_ID];
...
function setupEventListeners() {
    if (chatChangedBound.value) return;
    eventSource.on(event_types.CHAT_CHANGED, onChatChanged);
    chatChangedBound.value = true;
}
...
function processSingleMessage(mesElement) {
    ...
    if ($avatarWrapper.length && $chName.length && $targetSibling.length &&
        ...
        if ($nameText.length && !$nameText.parent().hasClass('immersive-vertical-wrapper')) {
            const $verticalWrapper = $('<div class="immersive-vertical-wrapper" style="display: flex; flex-direction: column; flex: 1; margin-top: 5px; align-self: stretch; justify-content: space-between;"></div>');
            const $topGroup = $('<div class="immersive-top-group"></div>');
...
function bindSettingsEvents() {
    ...
    setTimeout(() => {
        const checkbox = document.getElementById('immersive_layout_enabled');
...
function bindMessageEvents() {
    ...
    messageEventHandlers = [
        { eventType: event_types.MESSAGE_SENT, handler: onUserMessage },
        { eventType: event_types.MESSAGE_RECEIVED, handler: onAIMessage },
        { eventType: event_types.MESSAGE_DELETED, handler: onMessageChange },
        { eventType: event_types.MESSAGE_UPDATED, handler: onMessageChange },
        { eventType: event_types.MESSAGE_SWIPED, handler: onMessageChange },
        { eventType: event_types.GENERATION_ENDED, handler: onAIMessage },
        { eventType: event_types.MORE_MESSAGES_LOADED, handler: onMoreMessagesLoaded },
    ];
    messageEventHandlers.forEach(({ eventType, handler }) => eventSource.on(eventType, handler));
    state.messageEventsBound = true;
}

function unbindMessageEvents() {
    if (!state.messageEventsBound) return;
    messageEventHandlers.forEach(({ eventType, handler }) => eventSource.removeListener(eventType, handler));
    messageEventHandlers = [];
...
function restoreAvatarWrappers() {
    $(SEL.mes).each(function () {
        const $mes = $(this);
        const $avatarWrapper = $mes.find('.mesAvatarWrapper');
        const $verticalWrapper = $mes.find('.immersive-vertical-wrapper');
...
function showNavigationButtons($targetMes) {
    ...
    const $verticalWrapper = $targetMes.find('.immersive-vertical-wrapper');
...
function cleanup() {
    ...
    if (chatChangedBound.value) {
        eventSource.removeListener(event_types.CHAT_CHANGED, onChatChanged);
        chatChangedBound.value = false;
    }
